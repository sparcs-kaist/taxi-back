...
const createHandler = async (req, res) => {
  ...
  try {
    ...
    logger.info(await subscribeUserToTopic(user._id, `room-${room._id}`));
...
const abortHandler = async (req, res) => {
  ...
  try {
    ...
    // 방에서 나간 사용자를 `room-${room._id}` topic으로부터 구독 해제시킵니다.
    logger.info(await unsubscribeUserFromTopic(user._id, `room-${room._id}`));
