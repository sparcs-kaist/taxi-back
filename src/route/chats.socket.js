...
module.exports = (io, socket) => {
    ...
    socket.on("chats-disconnect", async () => {
        try {
            ...
            if(!myUser) return io.to(socket.id).emit("chats-disconnect", { err: "user not exist" });

            const roomId = session.chatRoomId;
            if(!roomId) return io.to(socket.id).emit("chats-disconnect", { err: "user not join chat room" });
    ...
    socket.on("chats-send", async (content) => {
        try {
            ...
            const roomId = session.chatRoomId;
            if(!roomId) return io.to(socket.id).emit("chats-send", { err: "user not join chat room" });
